...
export function addShapeToScene(ocLib, shape, scene) {
  const objectMaterial = new MeshStandardMaterial({
    color: new Color(0.9, 0.9, 0.9)
  });

  let geometries = visualize(ocLib, shape);

  let group = new Group();

  geometries.forEach(geometry => {
    group.add(new Mesh(geometry, objectMaterial));
  });
...
export function visualize(openCascade, shape) {
  let geometries = [];
